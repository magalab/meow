# Meow

[English](README.md) | [简体中文](README.zh-CN.md)

轻量级 macOS 启动器 + 挂件工具集（SwiftUI + AppKit）。

## 功能

- 全局快捷键（`Opt+Space`）呼出启动器
- 应用搜索（带启动历史权重）
- 剪贴板历史，支持粘贴、复制、删除与清空
- 原生区域、窗口和显示器截图，支持多屏冻结选择、全分辨率本地历史和可配置复制/保存输出
- 原生录屏，支持显示器、区域、窗口、应用、系统音频、摄像头和已连接的移动采集设备
- 可选桌面白板，支持图形、箭头、自由绘制、文字、图片、本地自动保存，以及 PNG/SVG/Excalidraw 交换
- 可选系统监控，使用独立菜单栏图标显示紧凑指标卡片，支持 IPv4 地址、采样间隔和历史时长配置
- S3 兼容文件上传，支持 AWS S3、Cloudflare R2、MinIO、自定义 Endpoint、上传快捷键、剪贴板命令和菜单栏拖拽
- 内置命令（偏好设置 / 询问 AI / 退出）
- 兼容 OpenAI Chat Completions 的 AI 聊天助手，并支持本地聊天历史
- 可从剪贴板条目直接询问 AI
- 选中文本翻译面板（需要辅助功能权限）
- 系统文本服务与 `Option+X` 操作面板，可翻译、询问 AI，并在 Miao 中朗读选中文本
- 离线语音识别，支持按住说话、自动粘贴和本地 WAV 历史
- 离线中英文语音合成，支持本地播放、WAV 导出和剪贴板朗读
- 按键可视化叠层，支持拖拽、显示模式、显示时长、透明度与历史数量
- 健康提醒，支持工作/休息计时、休息叠层、每日目标与轻量键鼠活跃检测
- 菜单栏日历，含农历日期、节气、节假日及 Calendar.app 事件
- TOTP 身份验证器，支持钥匙串存储、`otpauth://` 与 JSON 导入、JSON 备份、搜索和一键复制
- 使用正确签名构建时，可选择通过 iCloud 钥匙串同步身份验证器账户
- 7 种菜单栏日期图标样式（爪印、轮廓日期、圆角轮廓、仅日期、月+日、星期+日、农历）
- 3 种 Dock 图标样式（默认、日历、扁平）
- 条目操作菜单（打开、在 Finder 中显示、复制、粘贴、询问 AI、删除）
- 4 套主题配色：调皮猫猫、雾霾蓝、琥珀石墨、苔墨
- 运行时语言切换（英文 / 简体中文）
- 登录启动（受 macOS 签名策略限制）

## 环境要求

- macOS 15+
- Swift 6.0+

## 快速开始

```bash
# Debug 构建
swift build

# Release 构建
swift build -c release

# 运行
.build/debug/Meow

# 打包 DMG
bash scripts/build-dmg.sh
```

当 `logo.png` 比当前生成的 `AppIcon.icns` 更新时，DMG 构建脚本会自动重新生成应用图标。

Meow 提供两个构建版本：

```bash
# 基础版：Meow，不包含离线语音识别和语音合成
swift build -c release --product Meow
bash scripts/build-dmg.sh

# 语音版：Miao，包含离线语音识别和语音合成
MEOW_EDITION=voice swift build -c release --product Miao
MEOW_EDITION=voice bash scripts/build-dmg.sh
```

默认 Bundle ID 分别是：Meow 使用 `tech.lury.meow`，Miao 使用 `tech.lury.miao`。

如需自定义 Bundle ID：

```bash
APP_BUNDLE_ID=tech.lury.meow bash scripts/build-dmg.sh
```

## 使用方式

1. 启动 Meow 后，通过快捷键呼出面板（默认：`Opt+Space`）。
2. 输入关键词搜索应用或命令。
3. 使用 `上/下` 选择结果，回车启动应用或执行命令。
4. 在剪贴板条目上使用操作菜单，可粘贴、复制、删除、在 Finder 中显示，或询问 AI。
5. 在偏好设置中调整语言、主题、快捷键、Dock、菜单栏、剪贴板、白板、文件托管、身份验证器、健康提醒、按键叠层与 AI 设置。

## 选中文本操作

在支持 macOS 服务的应用中选中文字，然后从右键菜单或应用菜单选择「服务 -> 用 Meow 处理」（语音版显示为 Miao）。Meow 会打开统一操作面板，提供翻译和询问 AI；Miao 启用离线语音合成后还会显示朗读操作。

默认全局快捷键为 `Option+X`，可在「偏好设置 -> 通用 -> 快捷键」中修改。系统服务入口直接接收选中文字，不需要辅助功能权限；全局快捷键通过 Meow 的选区捕获回退机制读取文本，因此需要辅助功能权限。macOS 通常将入口放在「服务」子菜单中，Meow 无法强制它出现在所有应用的一级右键菜单。

Launch Services 从打包后的应用中发现该服务。测试右键入口时必须运行打包后的 `.app`，直接运行 `.build/debug/Meow` 无法完整验证。

## 截图

可在「偏好设置 -> 截图」中配置。

- 通过全局快捷键或启动器命令截取区域、窗口或整个显示器
- 从冻结的显示器画面中选择，避免动态内容在选择期间发生变化
- 支持复制、保存、复制并保存，可选择 PNG 或 JPEG
- 可选择默认快速截图模式，并分别配置快速、编辑、窗口和显示器截图快捷键
- 全分辨率截图保存在 `~/Library/Application Support/Meow/Captures/`
- 截图可直接进入 Meow 剪贴板历史，不受普通剪贴板图片尺寸限制
- 默认保存到 `~/Pictures/Meow/`，也可选择其他目录
- 可按数量、保存天数和本地存储容量清理截图历史
- 可拖动的截图后动作条支持复制、保存、编辑、贴图、OCR、翻译和询问 AI；可选择 5、10、20 秒或永不自动隐藏，悬停时暂停计时
- 编辑器支持裁剪、缩放、形状、箭头、画笔、文字、编号、高亮和不可逆打码
- 可将剪贴板图片固定为可缩放的置顶贴图
- 使用 Vision 在本地识别中英文，可复制文字，或连同源图预览发送到翻译面板；自动 OCR 历史索引默认关闭，开启后会将识别文字保存在本地以供搜索，内容可能包含敏感信息
- 扫描二维码，并在确认后将 `otpauth://` 账户安全导入钥匙串
- 仅在明确启用视觉能力并逐次确认后，向 OpenAI 兼容模型发送压缩后的真实图片数据
- 可在截图历史中搜索、编辑、复制、贴图、OCR、翻译、扫码、询问 AI、定位和删除截图；统一「历史」页支持网格/列表切换

截图需要屏幕录制权限。区域选择支持多显示器，按 `Esc` 可取消当前截图。屏幕录制与辅助功能权限相互独立。

## 桌面白板

桌面白板默认关闭。可在「偏好设置 -> 白板」中启用，然后通过 `Option+Shift+W`、
启动器命令或现有菜单栏入口进入和退出编辑模式。

- 支持矩形、椭圆、菱形、箭头、直线、自由画笔和文字
- 支持选择、移动、缩放、旋转、复制、粘贴、擦除、撤销和重做
- 支持画布平移、缩放、适配视图、图片导入，以及从剪贴板或截图发送图片
- 支持 Excalidraw 导入导出并保留未知字段，也可导出 PNG 和 SVG
- 可配置空闲时隐藏或保留内容，以及截图和录屏是否包含白板内容
- 编辑底色可选透明或纸张白，并与点阵/网格辅助线独立配置；PNG/SVG 导出也可独立选择透明或纸张白背景
- 功能关闭时完全休眠：不创建白板窗口、不读取工作区、不注册快捷键

首期同一时间只使用一个活动显示器。工作区和图片资源保存在：

```text
~/Library/Application Support/<Meow 或 Miao>/Whiteboards/
```

## 系统监控

系统监控默认关闭。可在「偏好设置 -> 系统监控」中启用；它使用独立的菜单栏图标，
不会和 Meow 主菜单栏图标混在一起。

- 显示 CPU、内存、GPU 内存、网络、磁盘、电池和温度状态
- 可配置菜单栏显示样式、采样间隔、历史时长和需要显示的指标
- 使用紧凑卡片显示指标、CPU/内存近期摘要，以及局域网/公网 IPv4 地址
- IP 地址提供一键复制，并在复制成功后显示视觉反馈
- 关闭时不启动采样服务；公网 IPv4 获取失败会使用退避策略重试

系统监控不需要额外的系统权限。局域网和公网地址均只显示 IPv4，IPv6 地址会被主动忽略。

## 文件上传

可在「偏好设置 -> 文件托管」中配置 S3 兼容文件托管。

- 支持保存 AWS S3、Cloudflare R2、MinIO 和自定义 S3 兼容 Endpoint 配置
- Secret Access Key 保存在 macOS 钥匙串中，非敏感配置保存在 Meow 设置中
- 可从截图后动作条或独立区域截图快捷键上传截图
- 可通过 Launcher 命令上传剪贴板中的文件或图片
- 可将文件拖到菜单栏图标上传，右键菜单不额外添加上传项
- 普通上传采用流式读取，64 MB 及以上文件切换为 multipart 上传
- 支持自定义域名、公共 Bucket 和预签名链接，并可复制 URL、Markdown 或 HTML
- 本地上传历史包含缩略图、来源标签、预签名链接刷新，以及可选的远端对象删除
- 上传完成后自动复制链接，并显示应用内成功提示

上传历史保存在：

```text
~/Library/Application Support/Meow/Uploads/
```

不同 S3 兼容服务在 Endpoint、签名、公开访问和 multipart 行为上可能不同。正式使用配置前，
请通过「测试上传」同时验证对象上传和最终分享链接访问。

## 录屏

可在「偏好设置 -> 录屏」中配置，并通过启动器命令或独立全局快捷键使用。

- 支持录制显示器、区域、单个或多个窗口、应用、系统音频和已连接的 iPhone/iPad 采集设备
- 支持 H.264、HEVC、透明 HEVC、SDR/HDR、Retina 分辨率，以及偏好设置中的最高 60 FPS
- 系统音频和指定麦克风可混合保存，也可保留为独立轨道
- 支持可移动摄像头叠层，并可选择矩形、圆角或圆形样式；同时支持系统原生点击高亮、跟随鼠标的放大镜和录制中保存 PNG 帧
- 过滤窗口内容可使用桌面、透明或自定义纯色背景
- 菜单栏和浮动控制条显示录制时长，并提供暂停与停止操作
- 默认保存到 `~/Movies/Meow/`，元数据和缩略图索引保存在
  `~/Library/Application Support/Meow/Recordings/`
- 录屏库支持搜索、网格/列表切换、Finder 定位、删除，以及按数量、时间和容量自动清理
- 录制完成后显示浮动预览，可播放并进行非破坏性的首尾裁剪
- 异常中断时保留恢复元数据，并在下次启动时扫描可恢复文件

显示器、区域、窗口、应用和系统音频捕获需要屏幕录制权限；麦克风和摄像头会分别请求权限。
HDR 会自动使用 MOV + HEVC；透明输出需要 MOV + HEVC with Alpha，主要适用于窗口或应用录制。

## 身份验证器

可在「偏好设置 -> 身份验证器」中配置 TOTP 身份验证器。

- 账户密钥保存在 macOS 钥匙串中，不写入 Meow 设置
- 支持手动输入 Base32 密钥和标准 `otpauth://` 链接
- 支持导入 Meow 备份、令牌数组和 Keyden Vault JSON
- 在明确提示明文密钥风险后，可导出带版本信息的 JSON 备份
- 复制的验证码和导入的密钥不会进入 Meow 剪贴板历史
- 可通过 iCloud 钥匙串同步独立账户

iCloud 钥匙串同步需要稳定的 Apple 签名及相应 entitlement。未签名或临时签名构建会保持本地存储，并显示缺少能力的原因。导出的 JSON 含明文 TOTP 密钥，必须安全保管。

## 离线语音识别

离线语音识别仅在 Miao 语音版中提供。可在「偏好设置 -> 语音」中配置。

- 启用功能后，按住 `Option+R` 录音（最长 30 秒），松开后识别并粘贴到当前应用
- 使用 sherpa-onnx 与本地语音模型，全程本地运行，支持多语言 SenseVoice Small int8 和英文 Parakeet int8
- 仅在偏好设置中确认后下载约 230 MB 模型
- 支持中文、英语、日语、韩语和粤语
- 成功识别的文本与 WAV 录音默认在本机保留 30 天
- 需要麦克风权限；自动粘贴还需要辅助功能权限

语音历史和模型保存在：

```text
~/Library/Application Support/Meow/ASRHistory/
~/Library/Application Support/Meow/Models/ASR/
```

## 离线语音合成

离线语音合成仅在 Miao 语音版中提供。可在「偏好设置 -> 语音 -> 合成」中配置。

- 复用现有 sherpa-onnx 运行库，使用 Matcha 中英双语模型
- 支持中文、英文、中英混合文本和稳定单音色
- 仅在用户确认后按需下载约 140 MB 模型和声码器
- 支持播放、暂停、继续、停止和 WAV 导出
- 启用后可朗读选中文本，并可从文本剪贴板条目和启动器命令朗读文本
- 朗读选中文本快捷键在「偏好设置 -> 语音 -> 合成」中配置
- 朗读选中文本需要辅助功能权限
- 模型安装后全程离线运行

TTS 模型保存在：

```text
~/Library/Application Support/Meow/Models/TTS/
```

## 健康提醒

Meow 可以在专注工作和短暂休息之间循环提醒。可在「偏好设置 -> 健康」中配置。

- 开始工作计时，并在该休息时提醒你
- 在菜单栏日历面板中显示控制项，并提供浮动休息叠层
- 在本地记录今日已完成和已跳过的休息次数
- 休息期间检测到键盘或鼠标操作时，可暂停休息倒计时
- 支持温和和严格两种休息窗口模式

## 按键可视化

Meow 可以在可拖拽叠层中显示全局按键。可在「偏好设置 -> 键盘」中配置。

- 全局按键监听需要辅助功能权限
- 支持仅快捷键、快捷键 + 特殊键、全部按键三种显示模式
- 支持紧凑/醒目样式、预设或自定义位置、透明度、显示时长，以及 1-3 条历史记录
- 按键标签会尽量跟随当前 macOS 键盘布局

## AI 助手

Meow 内置兼容 OpenAI 的聊天助手。可在「偏好设置 -> AI」中配置：

- 接口地址，例如 `https://api.openai.com/v1/chat/completions`
- API Key
- 模型名称，可手动输入，也可从服务商的 `/models` 接口获取
- 当前模型是否支持图片输入
- 上传图片的最长边和 JPEG 质量

聊天历史保存在本机：

```text
~/Library/Application Support/Meow/AIChats/
```

API Key 仍保存在 Meow 的本地设置中。聊天历史可在 AI 设置页关闭、清空，或直接打开所在文件夹。图片默认仅保留在本机，只有用户主动使用“询问 AI”并确认目标接口与压缩参数后才会上传。

## 代码结构

- `Sources/App/MeowApp.swift`: 应用生命周期与窗口管理
- `Sources/ViewModels/LauncherViewModel.swift`: 搜索与排序逻辑
- `Sources/Views/`: 启动器、AI 聊天、身份验证器、偏好设置、翻译面板与 UI 组件
- `Sources/Theme.swift`: 主题配色系统
- `Sources/Services/`: 快捷键、状态栏、自动启动、剪贴板、文件上传、翻译、系统监控、语音识别、身份验证器、AI 聊天与持久化
- `Sources/Services/SystemMonitor/`: 指标采集器、采样 Actor、历史记录和监控模型
- `Sources/Models/`: 应用、剪贴板、文件托管、身份验证器与设置模型
- `Sources/Resources/`: 本地化资源
- `Tests/`: Swift Testing 自动化测试

## 说明

- 提交变更前运行 `swift test`、`swift build` 和 `swift build -c release`。
- 系统监控变更需要手动检查：启用/关闭、独立菜单栏图标、指标显隐、紧凑弹窗布局、IPv4-only 地址、复制反馈，以及中英文本地化。
- 身份验证器相关变更需要手动检查：钥匙串存储、剪贴板历史排除、JSON 风险提示，以及同步不可用时的本地回退。
- AI 相关变更需要手动检查：已配置/未配置状态、模型获取/手动输入、Enter 发送、Shift+Enter 换行、聊天历史，以及打开历史文件夹。
- 健康提醒相关变更需要手动检查：计时开始/暂停/继续、休息开始/跳过/完成、每日目标进度、活跃检测暂停倒计时，以及菜单栏日历面板控制。
- 按键叠层相关变更需要手动检查：辅助功能权限拒绝/已授权状态、快捷键冲突处理、拖拽/重置叠层，以及非美式键盘布局。
- 文件上传相关变更需要手动检查：AWS S3、R2 和 MinIO 配置、公共/预签名链接、超过 64 MB 的文件、取消、剪贴板临时文件清理、拖拽、远端删除和钥匙串凭据处理。
- 详细开发说明见 [DEVELOPMENT.md](DEVELOPMENT.md)。
